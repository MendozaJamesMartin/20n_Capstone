...
class TransactionsController extends Controller
{
    public function GetTransactionsList(Request $request)
    {
        ...
        $query = DB::table('transactions')
            ...
            ->leftJoin('outsiders', function ($join) {
                $join->on('transactions.entity_id', '=', 'outsiders.id')
                    ->where('transactions.entity_type', '=', 'outsider');
            })
            ->select(
                'transactions.*',
                DB::raw("CASE 
                    ...
                    WHEN transactions.entity_type = 'outsider' THEN outsiders.name
                    ELSE 'Unknown'
                END AS entity_name")
            )
            ->where('balance_due', '>', '0');
    ...
    public function InsertNewOutsiderTransaction(Request $request)
    {
        DB::beginTransaction();
        try {
            if ($request->isMethod('get')) {
                // Display the form for creating a transaction
                $fees = DB::table('fees')->get();
                return view('common.forms.transaction-form-outsider', compact('fees'));
            } elseif ($request->isMethod('post')) {
                $validated = $request->validate([
                ...
                $feeIds = implode(',', array_keys($fees));
                $quantities = implode(',', array_values($fees));
                Log::info('Stored Procedure Start');
                DB::statement("CALL OutsiderPayFees(?, ?, ?, ?)", [
                    $validated['name'],
                    $validated['contact'],
                    $feeIds,
                    $quantities,
                ]);
                Log::info('Stored Procedure Complete');
                Log::info('Payment Success');

                DB::commit();

                return redirect()->route('receipts.list')->with('success', 'Transaction marked as paid successfully!');
            }
        } catch (QueryException $e) {
            DB::rollBack();
            Log::error("Inserting New Transaction Error: " . $e->getMessage());
    ...
    public function GetOutsiderTransactionDetails($id)
    {
        $TransactionDetails = DB::table('outsiders as o')
            ->join('outsider_transaction_details as otd', 'o.id', '=', 'otd.outsider_id')
            ->join('transactions as t', 'otd.transaction_id', '=', 't.id')
        ...
        // Return the result to a view to display the details
        $receipt = Receipt::where('transaction_id', $id)->first();
        return view('common.transactions.outs-trans-details', compact('TransactionDetails', 'receipt'));
    }
